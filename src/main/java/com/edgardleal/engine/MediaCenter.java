...
//			janela.setModal(true);
	...
	/**Retorna a imagem correspondente a chave informada , 
	 * caso não exista uma imagen para o nome informado ,
	 * retorna null.
	 * 
	 * @param nome
	 * @return
	...
	public void run() {
		...
		try{
			for (int i = 0; i < listaTemporaria.size(); i++) 
				add(listaTemporaria.get(i),applet.getImage(new URL(applet.getDocumentBase(),listaTemporaria.get(i))));
			
		}catch(Exception ex){erro = true; /*Silenciador*/}
		...
		while(!getTracker().checkAll()){
			...
			try {
				Thread.sleep(100);
				System.out.println("Itens checkados " + total_ok + " , total de itens " + getImages().size());
			} catch (Exception e) {
				//Sinlenciador da exceção
			}//try
		}//while
